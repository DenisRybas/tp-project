...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.confirmed and auth and check_password_hash(user.password, auth.password):
            token = jwt.encode(
                {'uid': user.id, 'exp': datetime.datetime.utcnow() + datetime.timedelta(minutes=30)},
...
def edit_account():
    ...
    if request.method == 'POST':
        ...
        return jsonify(username=user.nickname,
                       subscribed_on_daily_phrase=user.subscribed_on_daily_phrase), 200
    else:
        return jsonify(result="invalid request method"), 400
