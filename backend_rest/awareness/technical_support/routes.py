...
from backend_rest.awareness.models import User, TechnicalSupportTicket, TechnicalSupportChat
...
@technical_support_blueprint.route("/technical_support_tickets/new", methods=["POST", "GET"])
@token_required
def create_ticket():
    if request.method == "POST":
        ...
        ticket = TechnicalSupportTicket(creator_id=user_id, title=title, content=content, is_resolved=is_resolved)
        ...
        return jsonify(
            result="Unsupported request method", code=400
        )


@technical_support_blueprint.route("/technical_support_tickets/<int:ticket_id>", methods=["GET"])
@token_required
def ticket(ticket_id):
    ...
    return jsonify(title=ticket.title, content=ticket.content, messages=messages_to_json)


@technical_support_blueprint.route("/technical_support_tickets/<int:ticket_id>", methods=["POST"])
@token_required
def make_message(ticket_id):
    ...
    chat_message = TechnicalSupportChat(ticket_id=ticket.id, user_id=user_id, message=message_json['message'])
