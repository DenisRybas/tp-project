...
def create_template_diary():
    ...
    if form.validate_on_submit():
        ...
        diary_1 = UserTemplateDiary(
            ...
            theme_id=theme_1.id,
            answer=form.answer_1.data,
        )
        diary_2 = UserTemplateDiary(
            user_id=int(current_user.get_id()),
            theme_id=theme_2.id,
            answer=form.answer_2.data,
        )
    ...
    return render_template("template_diary.html", title="New template diary", form=form)
...
def get_template_diary(template_diary_id):
    template_diary = UserTemplateDiary.query.filter_by(
        id=template_diary_id, user_id=int(current_user.get_id())
    ).first_or_404()
